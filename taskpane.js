...
function _saveToken(token, expiresInSec) {
  ...
  try {
    ...
    _state.lastGraphError = 'save fallito: ' + e.message;
  }
...
function _loadToken() {
  ...
  try {
    ...
    if (token && Date.now() >= exp) {
      _state.lastGraphError = 'token scaduto';
    } else if (!token) {
      _state.lastGraphError = 'token assente in localStorage';
    }
  } catch (e) {
    _state.lastGraphError = 'load fallito: ' + e.message;
  }
  return null;
}
