...
function openMailto(type) {
  ...
  if (type === 'bug') {
    subject = '[EmailChainGuard] Segnalazione problema';
    bodyIntro = 'Descrivi il problema riscontrato (cosa stavi facendo, cosa ti aspettavi, cosa è successo invece):\n\n\n\n---\n';
  } else if (type === 'suggestion') {
    subject = '[EmailChainGuard] Suggerimento';
    bodyIntro = 'Descrivi la funzione che vorresti vedere o come potremmo migliorare il plugin:\n\n\n\n---\n';
  } else {
    subject = '[EmailChainGuard] Contatto';
    bodyIntro = 'Scrivi qui il tuo messaggio:\n\n\n\n---\n';
  }

  // Info tecniche per aiutare il debug (non sensibili: solo versione, lingua, OS, browser)
  let techInfo = 'Informazioni tecniche (non modificare):\n';
  techInfo += `Versione plugin: 4.0.0\n`;
  techInfo += `Lingua: ${_state.lang || 'it'}\n`;
  try {
    techInfo += `Piattaforma: ${navigator.platform || 'n/d'}\n`;
    const ua = navigator.userAgent || '';
    techInfo += `User-Agent: ${ua.substring(0, 200)}\n`;
  } catch {}
  techInfo += `Graph attivo: ${_state.graphEnabled ? 'sì' : 'no'}\n`;

  const body = bodyIntro + techInfo;
  const mailtoUrl = `mailto:${supportEmail}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;

  // Apre il client email predefinito dell'utente
  try {
