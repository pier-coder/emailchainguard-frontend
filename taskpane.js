...
async function callBackend(addresses, conversationId) {
  ...
  const body = JSON.stringify({ addresses, conversation_id: conversationId });
...
function renderResults(data) {
  // ── Overall status ──
  const { overall_label, max_risk_score, suspect_count, domains, addresses_used } = data;
  ...
  document.getElementById('footer-addr').textContent =
    `${addresses_used.length} indirizzi · ${domains.length} domini`;
}
