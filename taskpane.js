...
// ────────────────────────────────────────────────────────────
function initMsal() {
  if (_state.msalInstance || !window.msal) return;
  try {
    _state.msalInstance = new msal.PublicClientApplication({
      auth: {
        clientId: CONFIG.AZURE_CLIENT_ID,
        authority: `https://login.microsoftonline.com/${CONFIG.AZURE_TENANT}`,
        redirectUri: window.location.origin + '/emailchainguard-frontend/auth-callback.html',
      },
      cache: { cacheLocation: 'localStorage' },
    });
  } catch (e) {
    console.warn('ECG: MSAL init failed', e);
  }
}

async function enableGraph() {
  initMsal();
  if (!_state.msalInstance) throw new Error('MSAL non inizializzato');
  try {
    const result = await _state.msalInstance.loginPopup({ scopes: CONFIG.SCOPES });
    _state.graphToken = result.accessToken;
...
async function getGraphToken() {
  if (!_state.msalInstance) initMsal();
  if (!_state.msalInstance) return null;
  const accounts = _state.msalInstance.getAllAccounts();
  if (!accounts.length) return null;
  try {
    const result = await _state.msalInstance.acquireTokenSilent({
...
async function fetchConversationCC(conversationId) {
  // Restituisce set di tutti gli indirizzi mai apparsi nei CC della conversazione
  const token = await getGraphToken();
  if (!token) return null;
  try {
    const url = `https://graph.microsoft.com/v1.0/me/messages?$filter=conversationId eq '${encodeURIComponent(conversationId)}'&$select=ccRecipients,toRecipients&$top=50`;
...
function loadSettings() {
  ...
  if (_state.graphEnabled) initMsal();
}
...
async function runScan() {
  ...
  try {
    ...
    if (convId && ccFiltrati.length > 0) {
      // Prova Graph API se attivo
      let knownCC = null;
      if (_state.graphEnabled) {
        knownCC = await fetchConversationCC(convId);
      }
      // Fallback: storage locale
      if (!knownCC) knownCC = getKnownCCForConversation(convId);
...
function renderResults(data, newSenderEmail, newCCAddrs) {
  ...
  document.getElementById('foot-status').textContent = `${domains.length} ${t('domains_analyzed')}`;
