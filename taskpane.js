...
const CONFIG = {
  ...
  ECG_API_KEY:  'MalibyStacy25_5', // <-- sostituisci con la tua chiave
...
async function fetchConversationCC(conversationId, currentInternetMessageId) {
  // Restituisce set di tutti i CC apparsi nelle email PRECEDENTI della conversazione
  // (escludendo l'email che stiamo guardando ora)
  const token = await getGraphToken();
  if (!token) return null;
  try {
    const url = `https://graph.microsoft.com/v1.0/me/messages?$filter=conversationId eq '${encodeURIComponent(conversationId)}'&$select=ccRecipients,toRecipients,internetMessageId,receivedDateTime&$orderby=receivedDateTime asc&$top=50`;
    const resp = await fetch(url, { headers: { Authorization: `Bearer ${token}` } });
    if (!resp.ok) return null;
    const data = await resp.json();
    const allCC = new Set();
    const messages = data.value || [];
    // Se conosciamo l'ID dell'email corrente, fermati quando la trovi
    // (per escludere anch'essa e le successive)
    for (const msg of messages) {
      if (currentInternetMessageId && msg.internetMessageId === currentInternetMessageId) {
        break;
      }
  ...
  try {
    ...
    if (convId && ccFiltrati.length > 0) {
      ...
      if (_state.graphEnabled) {
        const currentMsgId = item.internetMessageId || null;
        knownCC = await fetchConversationCC(convId, currentMsgId);
      }
