...
function _silentRefreshToken() {
  return new Promise((resolve, reject) => {
    if (!Office?.context?.ui?.displayDialogAsync) {
      reject(new Error('Dialog API non disponibile'));
      return;
    }
    // Usa la pagina ponte con un flag per refresh silenzioso
    const url = 'https://pier-coder.github.io/emailchainguard-frontend/auth-callback.html?silent=1';
    Office.context.ui.displayDialogAsync(url, { height: 40, width: 30, displayInIframe: false }, (asyncResult) => {
      if (asyncResult.status !== Office.AsyncResultStatus.Succeeded) {
        reject(new Error(asyncResult.error?.message || 'dialog open fail'));
        return;
      }
      const dialog = asyncResult.value;
      let resolved = false;
      ...
      dialog.addEventHandler(Office.EventType.DialogMessageReceived, async (arg) => {
        ...
        try {
          ...
          if (data.access_token) {
            ...
            resolved = true;
            try { dialog.close(); } catch {}
            reject(new Error(data.error_description || data.error || 'no token'));
          }
        } catch (e) {
          resolved = true;
          try { dialog.close(); } catch {}
      ...
      dialog.addEventHandler(Office.EventType.DialogEventReceived, (arg) => {
        if (!resolved) {
          resolved = true;
          clearTimeout(timer);
          reject(new Error('dialog chiuso'));
        }
      });
    });
  });
